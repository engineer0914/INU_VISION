...
class Yolo3DNode(Node):
    ...
    def estimate_rect_from_image_contour(self, roi_color, core_bbox, depth_mask, cls_name):
        """
        이미지 기반 contour를 메인으로 사용.
        CLAHE + Lab Scharr gradient로 edge 추출.
        PCA는 yaw 계산용이 아니라 rect 방향 검증용으로 사용.
        """
        ...
        for cnt in contours:
            ...
            # 후보 단계에서 YOLO bbox 밖으로 너무 많이 나가는 rect는 폐기
            ...
            if self.is_square_class(cls_name):
                # 정사각형은 최종 단계에서 YOLO BL 기준 yaw로 다시 계산한다.
                # 후보 선택 단계에서는 임시로 yaw_rect만 보관.
                yaw = yaw_rect
                yaw_mode = 'square_yolo_bl_0_90'
                pca_penalty = 0.0
            else:
                ...
                # 직사각형은 PCA와 rect yaw가 너무 다르면 후보 신뢰 낮음
    ...
    def make_lab_gradient_edge(self, roi_color):
        """
        색상 threshold가 아니라 Lab 채널별 gradient magnitude를 사용.
        CLAHE는 L 채널에만 적용.
        """
    ...
    def estimate_rect_from_depth_mask(self, depth_mask, cls_name, core_bbox=None):
        ...
        # depth fallback도 YOLO bbox 밖으로 나가지 않도록 clamp
    ...
    def clamp_rect_to_bbox(self, rect, bbox):
        """
        minAreaRect 결과의 4개 꼭짓점이 YOLO bbox 밖으로 나가지 않게 제한한다.

        rect: cv2.minAreaRect 결과, ROI 좌표계 기준
        bbox: (x1, y1, x2, y2), ROI 좌표계 기준 YOLO bbox
        """
    ...
    def rect_outside_bbox_ratio(self, rect, bbox):
        """
        rect 꼭짓점이 bbox 밖으로 얼마나 나갔는지 대략적인 비율로 계산.
        0이면 완전히 내부.
        """
    ...
    def square_yaw_from_yolo_bottom_left(self, rect, bbox):
        ...
        기준:
        - YOLO bbox의 왼쪽 아래점 (x1, y2)
        - YOLO bbox의 하단선, 즉 +x 방향

        방법:
        - 최종 rect의 네 변 중 YOLO bbox 왼쪽 아래점에 가장 가까운 변을 선택
        - 그 변과 수평선 사이의 사잇각을 계산
        - 결과는 0~90도
        """
...
#        angle = 90 - angle
    ...
    def draw_debug_text(
        ...
        cv2.putText(
            debug_img,
            f"yaw:{yaw:.2f} {yaw_mode} H:{height_mm:.1f}mm",
            (text_x, text_y + 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 255, 255),
        ...
        cv2.putText(
            debug_img,
            f"rect:{rect_source} pcaR:{pca_ratio:.2f} out:{outside_ratio:.3f}",
            (text_x, text_y + 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (255, 255, 0),
