...
class Yolo3DNode(Node):
    ...
    def image_callback(self, color_msg, depth_msg):
        ...
        for box in results.boxes:
            ...
            if selected is not None:
                ...
                yaw_mode = str(selected.get('yaw_mode', 'normal'))
                ...
                # 중심 안정화: image/depth rect 중심 + YOLO 중심 가중 평균
                final_cx_roi = (
            ...
            if valid_pose:
                ...
                if self.print_pose_each_frame:
                    self.get_logger().info(
                        ...
                        f"pcaR={pca_ratio:.2f}, H={height_mm:.1f}mm"
                    )
            else:
                cv2.putText(
                    debug_img,
        ...
        for cnt in contours:
            ...
            if self.is_square_class(cls_name):
                ...
                if pca_reliable and yaw_diff <= self.max_yaw_diff_deg:
                    ...
                    # 직사각형인데 PCA ratio가 낮으면 모양이 애매한 후보
                    pca_penalty = 1.0
                else:
                    # yaw 차이가 너무 크면 후보 제외
                    continue
            ...
            total_score = bbox_score + 0.8 * pca_penalty + 0.8 * overlap_score
    ...
    # Depth fallback rect
    # ============================================================
    def estimate_rect_from_depth_mask(self, depth_mask, cls_name):
        contours, _ = cv2.findContours(
            depth_mask,
            cv2.RETR_EXTERNAL,
            cv2.CHAIN_APPROX_SIMPLE
    ...
    def draw_debug_text(
        ...
        cv2.putText(
            debug_img,
            f"rect:{rect_source} pcaR:{pca_ratio:.2f}",
            (text_x, text_y + 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (255, 255, 0),
