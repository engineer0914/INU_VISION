...
class Yolo3DNode(Node):
    def __init__(self):
        ...
        # ROI 내부 LAB L채널에 CLAHE를 적용한 뒤 추가 대비/밝기 보정
        # L_eq = convertScaleAbs(L_eq, alpha=roi_contrast_alpha, beta=roi_contrast_beta)
        self.declare_parameter('roi_contrast_alpha', 1.10)
        self.declare_parameter('roi_contrast_beta', 0.0)
        ...
        # ============================================================
        self.declare_parameter('cv_main_text_scale', 0.72)
        self.declare_parameter('cv_main_text_thickness', 2)

        self.declare_parameter('cv_sub_text_scale', 0.45)
        self.declare_parameter('cv_sub_text_thickness', 1)
        self.declare_parameter('show_sub_debug_text', True)

        self.declare_parameter('cv_text_line_gap', 24)
        ...
        self.roi_contrast_alpha = float(self.get_parameter('roi_contrast_alpha').value)
        self.roi_contrast_beta = float(self.get_parameter('roi_contrast_beta').value)
        ...
        self.show_sub_debug_text = bool(self.get_parameter('show_sub_debug_text').value)
    ...
    def make_lab_gradient_edge(self, roi_color):
        ...
        # CLAHE 이후 L 채널에 약한 대비/밝기 보정을 추가한다.
        # alpha > 1.0이면 대비가 증가하고, beta는 밝기 offset이다.
        if abs(self.roi_contrast_alpha - 1.0) > 1e-6 or abs(self.roi_contrast_beta) > 1e-6:
            L_eq = cv2.convertScaleAbs(
                L_eq,
                alpha=self.roi_contrast_alpha,
                beta=self.roi_contrast_beta
            )
    ...
    def draw_debug_text(
        ...
        main_gap = max(14, int(self.cv_text_line_gap))
        sub_gap = max(12, int(main_gap * 0.72))
        ...
        # 텍스트는 기본적으로 남색 padding ROI bbox 오른쪽에 표시한다.
        # 오른쪽 공간이 부족하면 ROI 왼쪽으로 fallback한다.
        text_x = int(roi_x2 + 10)
        text_y = int(max(22, roi_y1 + 22))

        # 현재 scale 기준 대략적인 텍스트 폭. 화면 밖으로 많이 나가는 것을 방지한다.
        approx_text_w = int(max(260, 380 * max(main_scale, 0.6)))
        if text_x + approx_text_w > img_w:
            text_x = int(max(5, roi_x1 - approx_text_w - 10))

        # 표시할 줄 수에 맞춰 화면 아래쪽으로 넘어가지 않게 보정한다.
        if self.show_sub_debug_text:
            total_h = main_gap * 2 + sub_gap * 5 + 14
        else:
            total_h = main_gap * 2 + 14
        if text_y + total_h > img_h - 5:
            text_y = max(22, img_h - total_h - 5)

        line0 = text_y
        line1 = line0 + main_gap
        line2 = line1 + main_gap
        ...
        if not self.show_sub_debug_text:
            return
...
def main(args=None):
    ...
    try:
        ...
        try:
            node.get_logger().info("KeyboardInterrupt: shutting down yolo_3d_node")
        except Exception:
            pass
